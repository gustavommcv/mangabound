...
- `components/` are grouped by what they are for: `mapping/` (the volume editor), `queue/`, `settings/` (the process steps, the mangapress options, reset), `sharing/`, `shared/` (notices and other compositions used across screens), and `ui/` (the design-system primitives: button, input, tabs, and so on).
