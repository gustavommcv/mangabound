...
- **The title bar is fixed.** The window is a bar and, under it, the part that scrolls. The bar carries the app name and the Share button in the space the window's own buttons leave free; on macOS, where those buttons are on the left, the name starts further in. What is on the bar is clicked, and only its empty part drags the window. The bar draws no line under itself: the window's own buttons are drawn by the system over its whole height and would hide a line in its last row, so the line is the top edge of the part that scrolls, just below the bar.
- **The empty drop area is clickable.** While the queue is empty the whole area answers the pointer: it lights up under it, shows the hand, and a click anywhere in it opens a menu of two choices, "Choose files" and "Choose a folder", because a native dialog on Windows and Linux cannot pick both in one go. The words in it are the menu's button, so the keyboard has the same way in, with the arrows to move, Enter to choose and Escape to put the menu away. The menu does not block the rest of the window, and a click elsewhere closes it and does its own job. Once the queue has items, only the line under them opens the menu.
